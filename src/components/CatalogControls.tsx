  ...
  return (
    <div className="catalog-controls">
      ...
      <div className="filter-row">
        ...
        <label><span className="sr-only">Бренд</span><select className="catalog-brand-select" value={query.brand ?? ''} onChange={(event) => onChange({ ...query, brand: event.target.value })}><option value="">Все бренды</option>{brands.map((brand) => <option key={brand}>{brand}</option>)}</select></label>
