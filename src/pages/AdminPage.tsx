  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveAdminProduct(selected.id, {
        ...
        key_notes: selected.key_notes, key_accords: selected.key_accords,
        perfumers: selected.perfumers, launch_year: selected.launch_year,
        image_url: selected.image_url, details_source_url: selected.details_source_url,
        details_status: selected.details_status,
    ...
    <section className="admin-section">
      ...
      <div className="admin-catalog-layout">
        ...
        {selected ? <form className="admin-editor" onSubmit={save}>
          ...
          <div className="form-grid"><AdminField label="Бренд" value={selected.brand} onChange={(value) => change('brand', value)} /><AdminField label="Название" value={selected.name} onChange={(value) => change('name', value)} /><AdminField label="Фланкер" value={selected.flanker ?? ''} onChange={(value) => change('flanker', value || null)} /><AdminField label="Концентрация" value={selected.concentration ?? ''} onChange={(value) => change('concentration', value || null)} /><AdminField label="Объём, мл" type="number" value={String(selected.volume_ml)} onChange={(value) => change('volume_ml', Number(value))} /><AdminField label="Семейство" value={selected.fragrance_family ?? ''} onChange={(value) => change('fragrance_family', value || null)} /><AdminField label="Год выпуска" type="number" value={selected.launch_year ? String(selected.launch_year) : ''} onChange={(value) => change('launch_year', value ? Number(value) : null)} /><AdminField wide label="Изображение URL" value={selected.image_url ?? ''} onChange={(value) => change('image_url', value || null)} /><AdminField wide label="Источник характеристик" value={selected.details_source_url ?? ''} onChange={(value) => change('details_source_url', value || null)} /><AdminArea label="Описание" value={selected.description} onChange={(value) => change('description', value)} /><AdminArea label="Верхние ноты через запятую" value={selected.top_notes.join(', ')} onChange={(value) => change('top_notes', split(value))} /><AdminArea label="Ноты сердца через запятую" value={selected.heart_notes.join(', ')} onChange={(value) => change('heart_notes', split(value))} /><AdminArea label="Базовые ноты через запятую" value={selected.base_notes.join(', ')} onChange={(value) => change('base_notes', split(value))} /><AdminArea label="Ключевые ноты через запятую" value={selected.key_notes.join(', ')} onChange={(value) => change('key_notes', split(value))} /><AdminArea label="Аккорды через запятую" value={selected.key_accords.join(', ')} onChange={(value) => change('key_accords', split(value))} /><AdminArea label="Парфюмеры через запятую" value={selected.perfumers.join(', ')} onChange={(value) => change('perfumers', split(value))} /></div>
          <div className="admin-options"><label>Карточка <select value={selected.details_status} onChange={(event) => change('details_status', event.target.value as AdminProduct['details_status'])}><option value="missing">Нет данных</option><option value="partial">Заполнена частично</option><option value="review">Проверить</option><option value="verified">Проверена</option></select></label><label>Цена <select value={selected.price_mode} onChange={(event) => change('price_mode', event.target.value as AdminProduct['price_mode'])}><option value="auto">Автоматическая</option><option value="manual">Ручная</option></select></label>{selected.price_mode === 'manual' && <label>Цена, ₽ <input required min="1" type="number" value={selected.manual_price_rub ?? ''} onChange={(event) => change('manual_price_rub', event.target.value ? Number(event.target.value) : null)} /></label>}<label>Наличие <select value={selected.availability} onChange={(event) => change('availability', event.target.value as AdminProduct['availability'])}><option value="in_stock">В наличии</option><option value="out_of_stock">Нет в наличии</option><option value="review">Проверить</option></select></label><label className="admin-check"><input type="checkbox" checked={selected.published} disabled={selected.details_status !== 'verified'} onChange={(event) => change('published', event.target.checked)} /> Показывать на сайте</label></div>
