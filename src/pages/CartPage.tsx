...
export function CartPage() {
  ...
  const blocked = resolved.filter(({ product }) => !getOrderReadiness(product).ready);
  ...
  return (
    <main className="cart-page">
      ...
      <div className="cart-layout"><section className="cart-lines" aria-label="Товары в корзине">{resolved.map(({ line, product }) => <article className="cart-line" key={product.id}><ProductImage src={product.imageUrl} alt="" variant="compact" /><div className="cart-line__name"><small className="product-brand-text">{product.brand}</small><Link className="product-name-text" to={`/product/${product.slug}`}>{product.name}</Link><span>{product.volumeMl ? `${product.volumeMl} мл` : 'Объём уточнить'}</span></div><div className="quantity" aria-label={`Количество ${product.name}`}><button type="button" onClick={() => cart.setQuantity(product.id, line.quantity - 1)} aria-label={`Уменьшить количество ${product.name}`}><Minus size={14} /></button><span>{line.quantity}</span><button type="button" onClick={() => cart.setQuantity(product.id, line.quantity + 1)} aria-label={`Увеличить количество ${product.name}`}><Plus size={14} /></button></div><strong>{product.priceRub ? `${rubles.format(product.priceRub * line.quantity)} ₽` : 'Уточнить'}</strong><button className="remove-button" type="button" onClick={() => cart.remove(product.id)} aria-label={`Удалить ${product.name}`}><Trash2 size={17} /></button></article>)}</section>
        <aside className="cart-summary"><p className="eyebrow">Итого</p><div><span>Позиций</span><b>{cart.itemCount}</b></div><div><span>Известная сумма</span><b>{rubles.format(cart.knownSubtotal)} ₽</b></div>{cart.unknownPriceCount > 0 && <p>{cart.unknownPriceCount} поз. с ценой по запросу</p>}{blocked.length > 0 ? <><div className="cart-blocked" role="status">{blocked.map(({ product }) => { const { missing } = getOrderReadiness(product); const fields = missing.map((item) => item === 'price' ? 'цену' : 'объём').join(' и '); return <p key={product.id}>Уточните {fields} у менеджера перед оформлением.</p>; })}</div><a className="button" href="https://t.me/jardinmanager" target="_blank" rel="noreferrer">Уточнить у менеджера <ArrowRight size={16} /></a></> : <Link className="button" to="/checkout">Оформить заказ <ArrowRight size={16} /></Link>}<button className="clear-button" type="button" onClick={cart.clear}>Очистить корзину</button></aside>
      </div>
    </main>
  );
}
